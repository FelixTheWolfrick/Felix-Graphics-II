...
a3ret a3vertexDrawableRenderIsoPatches(a3ui32 const count)
{
	if (count)
	{
		...
		float const tessLevelOuter[4] = {1.0f, 1.0f, 1.0f, 1.0f};
		float const tessLevelInner[2] = {1.0f, 1.0f};
		...
		glBindVertexArray(0); // turn off anything related to vertices
		glBindBuffer(GL_ARRAY_BUFFER, 0);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
		glDrawArrays(GL_PATCHES, 0, 2 * count); // 2 vertices for every segment, doing patches not lines
		return 1;
	}
	return -1;
}
