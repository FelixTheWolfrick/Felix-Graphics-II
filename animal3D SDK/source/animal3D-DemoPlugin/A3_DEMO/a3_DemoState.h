...
struct a3_DemoState
{
	...
	// draw data buffers (data containers)
	union {
	...
	// vertex array objects (Describe data to GPU, provides context)
	union {
	...
	// drawables (Puts buffer & description together)
	union { //Every member inside a union will have the same address
